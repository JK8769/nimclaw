...
proc initMaster*(m: var Master; timeout = default(Duration); activeProducer = false) =
  ## Initialize a Master in-place, avoiding =sink.
  initCond(m.c)
  initLock(m.L)
...
template spawn*[T: not void](master: var Master; fn: T): TaskVar[T] =
  block:
    var res = TaskVar[T]()
    if stillHaveTime(master):
      if shouldSend(master):
...
template spawn*[T: not void](m: MasterHandle; fn: T): TaskVar[T] =
  spawn(m.x[], fn)
