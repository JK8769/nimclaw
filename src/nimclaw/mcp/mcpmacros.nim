...
proc generateInputSchema(params: NimNode, paramDescs: Table[string, string]): JsonNode =
  ...
  for i in 1..<params.len:
    ...
    if param.kind == nnkIdentDefs:
      ...
      let hasDefault = param[^1].kind != nnkEmpty
      ...
      if param.kind == nnkIdentDefs:
        ...
        for j in 0..<param.len-2:
          ...
          if paramName != "":
            ...
            let hasDefault = param[^1].kind != nnkEmpty
            let getter = if hasDefault:
                           let defaultVal = param[^1]
                           quote do:
                             if `jsonArgsIdent`.hasKey(`paramNameLit`):
                               `jsonArgsIdent`[`paramNameLit`]
                             else:
                               %(`defaultVal`)
                         else:
                           quote do: `jsonArgsIdent`[`paramNameLit`]

            case $paramType:
            of "int", "int8", "int16", "int32", "int64":
              argExtractions.add(newCall(bindSym("getInt"), getter))
            of "uint", "uint8", "uint16", "uint32", "uint64":
              argExtractions.add(newCall(bindSym("getInt"), getter))
            of "float", "float32", "float64":
              argExtractions.add(newCall(bindSym("getFloat"), getter))
            of "string":
              argExtractions.add(newCall(bindSym("getStr"), getter))
            of "bool":
              argExtractions.add(newCall(bindSym("getBool"), getter))
            else:
              argExtractions.add(newCall(bindSym("getStr"), getter))
