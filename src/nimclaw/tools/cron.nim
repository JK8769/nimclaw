...
method description*(t: CronTool): string = "The ONLY tool you should use to schedule time-based reminders and tasks. IMPORTANT: When user asks to be reminded or scheduled, you MUST call this tool. Use 'at_seconds' for one-time reminders (e.g., 'remind me in 10 minutes' → at_seconds=600). Use 'every_seconds' ONLY for recurring tasks (e.g., 'every 2 hours' → every_seconds=7200). Use 'cron_expr' for complex recurring schedules (e.g., '0 9 * * *' for daily at 9am)."
method parameters*(t: CronTool): Table[string, JsonNode] =
  {
    ...
    "properties": %*{
      "action": {
        "type": "string",
        "enum": ["add", "create", "list", "remove", "update", "enable", "disable"],
        "description": "Action to perform. Use 'add' or 'create' when user wants to schedule a new reminder or task. Use 'update' to modify an existing one."
      },
      "message": {
        "type": "string",
        "description": "The reminder/task message to display when triggered (required for add, optional for update)"
      },
      "at_seconds": {
        "type": "integer",
      ...
      "job_id": {
        "type": "string",
        "description": "Job ID (for remove/update/enable/disable)"
      },
      "enabled": {
        "type": "boolean",
        "description": "Enable or disable the job (for update action only)"
      },
      "deliver": {
        "type": "boolean",
...
proc updateJob(t: CronTool, args: Table[string, JsonNode]): string =
...
method execute*(t: CronTool, args: Table[string, JsonNode]): Future[string] {.async.} =
  ...
  case action:
  of "add", "create": return await t.addJob(args)
